...
public class LordToil_BillyCaravan_GatherAnimals : LordToil_PrepareCaravan_GatherAnimals
{
    ...
    {
        data = new LordToilData_BillyCaravan_GatherPawns(meetingPoint);
    }
    ...
    private static bool IsFastAnimalCollectionEnabled => LoadedModManager.GetMod<CaravanModInit>()
        .GetSettings<CaravanModSettings>().fastAnimalCollection;

    public virtual IntVec3 MeetingPoint
    {
        get => ((LordToilData_BillyCaravan_GatherPawns)data).meetingPoint;
        set
        {
            ((LordToilData_BillyCaravan_GatherPawns)data).meetingPoint = value;
            ((LordToilData_BillyCaravan_GatherPawns)data).gatheredPawns.Clear();
        }
    }
    ...
    public override void UpdateAllDuties()
    {
        foreach (var pawn in lord.ownedPawns)
        {
            ...
            {
                pawn.mindState.duty = new PawnDuty(DutyDefOf.PrepareCaravan_CollectAnimals, MeetingPoint);
            }
            else
            {
    ...
    public override void LordToilTick()
    {
        ...
        {
            (data as LordToilData_BillyCaravan_GatherPawns)?.CheckGathered(lord, toilCompleteMemo,
                x => x.RaceProps.Animal, true);
        }
    }
}
...
//    {
//        get => (data as LordToilData_BillyCaravan_GatherPawns).meetingPoint;
//        set
//        {
...
//        if (Find.TickManager.TicksGame % 100 == 0)
//        {
//            (data as LordToilData_BillyCaravan_GatherPawns).CheckGathered(lord, "AllSlavesGathered",
//                x => !x.IsColonist && !x.RaceProps.Animal, true);
//        }
//    }
//}
...
public class LordToilData_BillyCaravan_GatherPawns : LordToilData
{
    ...
    {
        gatheredPawns = new List<Pawn>();
        meetingPoint = IntVec3.Invalid;
    }

    public override void ExposeData()
    {
        Scribe_Collections.Look(ref gatheredPawns, "gatheredPawns", LookMode.Reference);
    ...
    public void CheckGathered(Lord lord, string memo, Predicate<Pawn> shouldCheck, bool validateFollow)
    {
        ...
        {
            if (shouldCheck(pawn) &&
                (!validateFollow || GatherAnimalsAndSlavesForCaravanUtility.IsRopedByCaravanPawn(pawn)) &&
                pawn.Spawned &&
...
public static class Extend_Transition
{
    public static Transition ReplaceState(this Transition self, LordToil old_state, LordToil new_state)
    {
        ...
        var index = self.sources.IndexOf(old_state);
        if (index != -1)
        {
...
public static class Extend_StateGraph
{
    ...
    {
        var index = self.lordToils.IndexOf(old_state);
        while (index != -1)
        {
...
{
    [HarmonyPostfix]
    private static void Postfix(LordJob_FormAndSendCaravan __instance, ref StateGraph __result)
    {
        ...
        var tthis = Traverse.Create(__instance);
        var meetingPoint = tthis.Field("meetingPoint").GetValue<IntVec3>();

        // All we need to do is replace the GatherAnimals and GatherSlaves LordToils with our versions.

        // OLD VERSIONS
        var oldGatherAnimals = tthis.Field("gatherAnimals").GetValue<LordToil>();
        //var oldGatherSlaves = tthis.Field("gatherSlaves").GetValue<LordToil>();

        // OUR NEW VERSIONS
        var billyGatherAnimals = new LordToil_BillyCaravan_GatherAnimals(meetingPoint);
        //var billyGatherSlaves = new LordToil_BillyCaravan_GatherSlaves(meetingPoint);
